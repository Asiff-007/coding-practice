...
public class EventLiveData<T> extends LiveData<T> {
    ...
    public EventLiveData() {
        this.internalObserver = (new Observer<T>() {
            ...
            public void onChanged(T t) {
                Iterator<Map.Entry<Observer<? super T>, EventObserverWrapper>> iterator = io.thoughtbox.asif.util.EventLiveData.this.observers.entrySet().iterator();
    ...
    public void observeForever(@NonNull Observer observer) {
        ...
        EventObserverWrapper existing = wrapper;
        if (!observers.containsKey(observer)) existing = observers.put(observer, wrapper);
        if (existing != null && existing instanceof io.thoughtbox.asif.util.EventLiveData.EventLifecycleBoundEventObserver) {
            throw new IllegalArgumentException("Cannot add the same observer"
                    + " with different lifecycles");
        }
        if (existing != null) {
    ...
    public void removeObservers(@NonNull LifecycleOwner owner) {
        ...
        while (iterator.hasNext()) {
            ...
            if (entry.getValue() instanceof io.thoughtbox.asif.util.EventLiveData.EventLifecycleBoundEventObserver) {
    ...
    private abstract class EventObserverWrapper {
        ...
        void activeStateChanged(boolean newActive) {
            ...
            // owner
            mActive = newActive;
            boolean wasInactive = io.thoughtbox.asif.util.EventLiveData.this.mActiveCount == 0;
            io.thoughtbox.asif.util.EventLiveData.this.mActiveCount += mActive ? 1 : -1;
            if (wasInactive && mActive) {
                onActiveEvent();
            }
            if (io.thoughtbox.asif.util.EventLiveData.this.mActiveCount == 0 && !mActive) {
                onInactive();
            }
        }
    }
